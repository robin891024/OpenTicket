...
public interface EventTicketTypeRep extends JpaRepository<EventTicketType, Long> {
    ...
    // 依 TicketType 查詢
    EventTicketType findByTicketTypeId(Long ticketTypeId);
    ...
    @Query("select e from EventTicketType e where e.id = :id")
    EventTicketType findByIdForUpdate(@Param("id") Long id);
}
